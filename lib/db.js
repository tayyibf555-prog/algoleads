...
const SCHEMA = [
  ...
  // ──────────────────────────────────────────────────────────────
  // Members — owned by the member portal (writes happen there only).
  // The admin keeps this DDL as a CREATE IF NOT EXISTS purely so
  // local dev (separate SQLite file) can SELECT against it without
  // crashing. In production, both portals share the same Turso DB
  // and the table is created by whichever portal cold-starts first.
  // The admin must NEVER write to this table — it only reads to
  // surface who's signed up to the member portal.
  // ──────────────────────────────────────────────────────────────
  ...
  // Broker account details — owned by the member portal (writes
  // happen there only). The admin reads this table via a LEFT JOIN
  // in the /api/members endpoint to surface broker info per member.
  // DDL duplicated here only so local-dev SELECTs don't crash; in
  // production both portals share the same Turso DB.
